...
(TODO: introduce and explain choice of mirrorbits)

=== JEP versus IEP

We chose to open a JEP instead of a IEP in https://github.com/jenkins-infra/iep as we think the scope of this project is larger than just the infrastructure aspect.
We'll require for example that the Core must follow UC redirections introduced by mirrorbits.
